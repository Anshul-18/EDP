...
try:
    ...
    print("Warning: Could not load existing explainer, creating new one...")
    # Create a new explainer if loading fails
    from lime import lime_tabular
    # Get some sample data for training the explainer
    sample_features = classifier.predict_proba(np.zeros((1,39)))  # Create dummy data
...
def classify(features):
    ...
    proba_risk = sum(list(proba[0,2:]))
    if proba_risk >0.6: risk = ["<p style=\"color:red;\">Very High</p>"]
    elif proba_risk >0.4: risk = ["<p style=\"color:orangered;\">High</p>"]
    if proba_risk >0.2: risk = ["<p style=\"color:orange;\">Medium</p>"]
    if proba_risk >0.1: risk = ["<p style=\"color:green;\">Low</p>"]
